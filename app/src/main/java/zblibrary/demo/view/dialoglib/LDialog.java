package zblibrary.demo.view.dialoglib;
...
//    >>>>>>>>>>>>>>>>>>>>>>>>>>>>设置监听>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    public LDialog setOnClickListener(final View.OnClickListener onClickListener, int... viewIds) {
        final LDialog lDialog2 = this;
        for (int i = 0; i < viewIds.length; i++) {
            getView(viewIds[i]).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
